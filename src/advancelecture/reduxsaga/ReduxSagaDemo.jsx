...
const ReduxSagaDemo = props => {
  ...
  const handleAddTodo = () => {
    if (input !== "") {
      // console.log('now requesting to add new todo from component');
  ...
  return (
    <div>
      ...
      {todos &&
        todos.map((e, i) => {
          return (
            <div
              key={i}
              className="an-20 dark light--text text-center py10 ma10 flex-x"
            >
              <div className="flex-1">{e.name}</div>
              <div
                className="an-14 redcolor--text mr10 cursor-pointer"
                onClick={() => props.requestDeleteTodo(e.id)}
              >
                Delete
              </div>
            </div>
      ...
      <div className="text-center">
        ...
        />
        <div>
          <button className="pa10 cursor-pointer" onClick={handleAddTodo}>
            ADD TODO
          </button>
        </div>
      </div>
